...
- `BlackHole_simulation_main.html`
- `BlackHole_simulation_codex.html`

## 1) High-Level Architecture

| Area | do_not_change | main | codex |
|---|---|---|---|
...
| main | No lens post pass; raymarch integration bends rays (`uLensStrength` in acceleration) | Strength changes ray integration in disk/background shading | Physically coherent feel for disk, but no separate screen-space warp layer |
...
| main | Single particle pass | Analytic black-hole shadow ray test | Stable center occlusion, less wrap control |
...
| main | Procedural room coloring in raymarch shader | Emissive/procedural shading only |
...
| Control | do_not_change | main | codex |
|---|---|---|---|
...
- `main`: previously had `uSpinSpeed` and particle brightness default mismatches (first-frame mismatch risk).
...
- `main`: single-pass particles are simpler but less controllable for front/back wrap composition.
...
2. Keep raymarch gravity bend logic from `main` style as the primary physical lens response.
...
2. Port only the lensing pieces we prefer from `main` into codex (not full file copy).
